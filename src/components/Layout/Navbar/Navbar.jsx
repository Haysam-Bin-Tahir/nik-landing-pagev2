        ...
        <nav className='navbar'>
            ...
            <ul className="desktop-nav navbar-links">
                {navLinks.map((navLink, idx) => (
                    <li key={idx} className={clsx("nav-link", { "active" : (activeHref === navLink.href) })} onClick={handleHrefChanged(navLink.href)}>
                        <a href={navLink.href} onClick={(e) => scrollToElement(e, navLink.href)}>{navLink.label}</a>
                    </li>
                ))}
            </ul>
            ...
            <div className={clsx("sidebar-overlay", { 'show' : openMenu })} onClick={() => setOpenMenu(false)}>

            </div>
            <div className={clsx("sidebar", { 'show' : openMenu })}>
                <div className="btn-close">
                    <button className="btn-nav-menu btn-menu-close" onClick={() => setOpenMenu(false)}>
                ...
                <ul className="mobile-menu navbar-links">
                    {navLinks.map((navLink, idx) => (
                        <li key={idx} className={clsx("nav-link", { "active" : (activeHref === navLink.href) })} onClick={handleHrefChanged(navLink.href)}>
                            <a href={navLink.href} onClick={(e) => scrollToElement(e, navLink.href, openMenu)}>{navLink.label}</a>
                        </li>
                    ))}
                </ul>
            </div>
